...
class RnaInterference(ProcessBase):
    ...
    def __init__(self, config=None):
        ...
        # binding probability)
        self.srna_ids = self.config["srna_ids"]
        self.target_tu_ids = self.config["target_tu_ids"]
        self.binding_probs = self.config["binding_probs"]
        self.ribosome30S = self.config["ribosome30S"]
        self.ribosome50S = self.config["ribosome50S"]
        self.duplex_ids = list(self.config["duplex_ids"])
