...
def main(config_path: Path | None = None, duration: float | None = None, consolidate_metadata: bool = True):
    config_path = config_path or Path(CONFIG_DIR_PATH) / "tfba_steps.json"
    store_path = Path(OUT_DIR) / "tfba_steps.zip"

    sim = simulate(config_path, max_duration=duration)
    export: xr.DataTree = export_metabolism(sim)
    ...
    with warnings.catch_warnings():
        ...
        export.to_zarr(store, zarr_format=2 if consolidate_metadata else 3, consolidated=consolidate_metadata)
        if consolidate_metadata:
            zr.consolidate_metadata(store)
    store.close()


def test_main():
    confpath = Path('/Users/alexanderpatrie/sms/vecoli_fork/configs/dfba_steps.json')
    dur = 10800.0
    main(confpath, dur)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--duration", default=1111.0, type=float)
    parser.add_argument("--consolidated", required=False, type=bool, default=True, help="Whether to consolidate zarr metdatadata. Defaults to True")
    args = parser.parse_args()
    main(args.duration, args.consolidated)
