...
def plot_stoich_timeseries_network(
    ...
    *,
    ...
    for k, y in zip(keys, arrays):
        ...
        # Remove x-axis from reactants, keep only for product
    ...
    # Uncomment this block if you want arrows back
    # for u, v in G.edges():
...
def plot(
    ...
    # Map bulk molecule IDs -> display gene names
    species_to_gene_name = {
        "FLIF-FLAGELLAR-MS-RING[i]": "FLIF",
        ...
        "FLGB-FLAGELLAR-MOTOR-ROD-PROTEIN[j]": "FLGB",
        "FLGC-FLAGELLAR-MOTOR-ROD-PROTEIN[j]": "FLGC",
        "FLGF-FLAGELLAR-MOTOR-ROD-PROTEIN[j]": "FLGF",
        "FLGG-FLAGELLAR-MOTOR-ROD-PROTEIN[o]": "FLGG",
        "FLGI-FLAGELLAR-P-RING[j]": "FLGI",
        "FLGH-FLAGELLAR-L-RING[j]": "FLGH",
        "EG10322-MONOMER[j]": "FLIL",
        "MOTA-FLAGELLAR-MOTOR-STATOR-PROTEIN[i]": "MOTA",
        "MOTB-FLAGELLAR-MOTOR-STATOR-PROTEIN[i]": "MOTB",
        "FLAGELLAR-MOTOR-COMPLEX[j]": "FLAGELLAR_MOTOR_COMPLEX",
    }

    rxn_id = "FLAGELLAR_MOTOR_COMPLEX"
    ...
    product_species = "FLAGELLAR-MOTOR-COMPLEX[j]"

    # Build gene-name keyed dataframe
    ...
    fig, axes, G, pos = plot_stoich_timeseries_network(
        ...
        product_size_scale=1.05,
        title="Flagellar motor complex protein counts",
    )

    if product in axes:
        axes[product].set_title("Motor complex", fontsize=7, pad=2)
