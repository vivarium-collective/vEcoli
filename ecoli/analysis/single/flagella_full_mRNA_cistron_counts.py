...
# THIS FILE IS JUST FULL MRNA CISTRON COUNTS LISTENER TO SEE IF THE DATA IS SAME OR DIFFERENT
...
def plot(
    ...
    query = f"""
        SELECT listeners__rna_counts__full_mRNA_cistron_counts,time FROM ({history_sql})
        ORDER BY time
    """
    ...
    matrix = np.stack(
        output_df["listeners__rna_counts__full_mRNA_cistron_counts"].values
    ).astype(int)
