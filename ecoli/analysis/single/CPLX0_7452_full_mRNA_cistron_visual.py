...
# for full mRNA cistron counts corresponding to the final flagellum
# reaction (CPLX0-7452_RXN), using gene names and the same order/layout
# as the protein-count network figure.
...
def left_right_star_layout_grid(
    ...
    *,
    ...
    # Preserve the exact input reactant order instead of sorting,
    # so the layout matches your protein figure.
...
def plot_stoich_timeseries_network(
    ...
    *,
    ...
    # Arrows intentionally left commented out
    # for u, v in G.edges():
...
def plot(
    ...
    query = f"""
        SELECT listeners__rna_counts__full_mRNA_cistron_counts, time
        FROM ({history_sql})
        ORDER BY time
    """
    ...
    mrna_matrix = np.stack(
        output_queries["listeners__rna_counts__full_mRNA_cistron_counts"].values
    ).astype(float)
    ...
    df_full = pd.DataFrame(mrna_matrix, columns=mrna_cistron_ids)
    df_full["Time (min)"] = time_mins

    # gene_id -> cistron_id mapping for mRNA cistrons only
    gene_id_to_cistron_id = {
        row["gene_id"]: row["id"] for row in cistron_data if row["is_mRNA"]
    }

    # Keep EXACT same displayed order as protein figure
    gene_name_to_gene_id = {
        ...
        "FLIC": "EG10321",
        "FLID": "EG10841",
        "MOTOR_COMPLEX": None,  # schematic placeholder; no mRNA cistron
        "FLAGELLUM": None,  # schematic product placeholder; no mRNA cistron
    }
    ...
    for gene_name, gene_id in gene_name_to_gene_id.items():
        ...
        if cistron_id is None:
            print(f"WARNING: {gene_name} ({gene_id}) not found in cistron_data")
        else:
            gene_name_to_cistron_id[gene_name] = cistron_id

    # Same order as protein figure
    reactants = ["FLGE", "FLGK", "FLGL", "FLIC", "FLID", "MOTOR_COMPLEX"]
    product = "FLAGELLUM"
    rxn_id = "CPLX0-7452_full_mRNA_cistrons"
    ...
    # MOTOR_COMPLEX is a schematic placeholder in the mRNA figure
    timeseries["MOTOR_COMPLEX"] = np.zeros_like(time_mins, dtype=float)

    for gene_name in ["FLGE", "FLGK", "FLGL", "FLIC", "FLID"]:
        cistron_id = gene_name_to_cistron_id.get(gene_name)
        if cistron_id is None:
            print(f"WARNING: no cistron mapped for {gene_name}")
            continue
        if cistron_id not in df_full.columns:
            print(f"WARNING: {gene_name} cistron {cistron_id} not found in dataframe")
            continue
        timeseries[gene_name] = df_full[cistron_id].to_numpy()

    # Product node is also schematic for layout consistency
    timeseries["FLAGELLUM"] = np.zeros_like(time_mins, dtype=float)

    t = df_full["Time (min)"].to_numpy()
    ...
    fig, axes, G, pos = plot_stoich_timeseries_network(
        ...
        t=t,
        node_size=0.16,
        title="Final Flagellum Assembly mRNA Counts",
        figsize=(14, 8),
        force_left_right_grid=True,
    ...
    if "FLAGELLUM" in axes:
        axes["FLAGELLUM"].set_title("Flagellum", fontsize=9, pad=2)
    if "MOTOR_COMPLEX" in axes:
        axes["MOTOR_COMPLEX"].set_title("Motor complex", fontsize=9, pad=2)
    ...
    fig.text(0.02, 0.5, "Full mRNA Cistron Count", va="center", rotation="vertical")
    plt.close(fig)
