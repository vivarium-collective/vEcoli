...
# This function returns a dict with cooridnates for each reactant
# coordinates calculated based on left_x_range and y_range and number of rows/cols
def left_right_star_layout_grid(
    reactants,
    product,
    *,
    left_x_range=(0.05, 0.52),
    y_range=(0.88, 0.08),
    ncols=4,
    nrows=4,
    right_x=0.78,
...
def plot_stoich_timeseries_network(
    ...
    *,
    ...
    for k, y in zip(keys, arrays):
        ...
        ax.set_title(k, fontsize=6, pad=2)
    ...
    # This computes where the arrows should start and end
    # u is reactants, v is product
    # cu and cv provide center of each subplot
    # start and end then trims the arrows so it touches the edge of the box and not the center
    # rv and ru are the rectangle of each subplot in figure coordinates
    # ru is the source or reactants node's subplot and rv is the rectangle of the target node (product)
    # rect_edge_intersection draws a line from center of u and center of v and find where that line exits rectangle ru (this becomes the arrows starting point) = start
    # end = start from cv (product center) and draw line toward center of u, find where it exits rect rv which is the entry point of product box (rv)
...
def plot(
    ...
    query = f"""
        SELECT listeners__rna_synth_prob__actual_rna_synth_prob_per_cistron, time
        FROM ({history_sql})
        ORDER BY time
    """
    ...
    df = pd.DataFrame(cistron_synth_matrix, columns=cistron_ids)
    df["Time (min)"] = time_mins

    # Map gene_id -> cistron_id for mRNA cistrons only
    gene_id_to_cistron_id = {
        row["gene_id"]: row["id"] for row in cistron_data if row["is_mRNA"]
    }

    # Gene IDs for flagellar motor assembly components
    ...
    missing_gene_ids = []
    for gene_name, gene_id in gene_name_to_gene_id.items():
        cistron_id = gene_id_to_cistron_id.get(gene_id)
        if cistron_id is None:
            missing_gene_ids.append((gene_name, gene_id))
        else:
            gene_name_to_cistron_id[gene_name] = cistron_id

    if missing_gene_ids:
        print("WARNING: these genes were not found in cistron_data:")
        for gene_name, gene_id in missing_gene_ids:
            print(f"  {gene_name}: {gene_id}")

    # Use cistron IDs as the network node IDs so the timeseries lookup works directly
    reactants = [
        gene_name_to_cistron_id[g]
        for g in [
            "FLIF",
            "FLIE",
            "FLIG",
        ...
        if g in gene_name_to_cistron_id
    ]

    # Dummy product node so the network layout/edges still render
    product = "FLAGELLAR_MOTOR_ASSEMBLY_TARGET"

    rxn_id = "flagellar_motor_complex_synth_prob"
    ...
    present = [n for n in reactants if n in df.columns]
    missing = [n for n in reactants if n not in df.columns]
    if missing:
        print("WARNING: missing cistrons (won't be plotted):", missing)

    timeseries = {n: df[n].to_numpy() for n in present}

    # Add dummy zero series for the product node
    timeseries[product] = np.zeros_like(time_mins, dtype=float)
    ...
    fig, axes, G, pos = plot_stoich_timeseries_network(
        ...
        product_size_scale=1.25,
        title="Flagellar motor assembly gene synthesis probabilities",
    )

    # Optional: relabel subplot titles from cistron IDs to gene names
    cistron_id_to_gene_name = {v: k for k, v in gene_name_to_cistron_id.items()}
    for node, ax in axes.items():
        if node == product:
            ax.set_title("Motor complex", fontsize=6, pad=2)
        elif node in cistron_id_to_gene_name:
            ax.set_title(
                (cistron_id_to_gene_name[node]),
                fontsize=6,
                pad=2,
            )

    fig.savefig(
        os.path.join(outdir, f"{rxn_id}_network_timeseries.png"),
        dpi=300,
        bbox_inches="tight",
    )
    ...
    fig.text(
        0.02,
        0.5,
        "Actual RNA synthesis probability per cistron",
        va="center",
        rotation="vertical",
    )
    plt.close(fig)
