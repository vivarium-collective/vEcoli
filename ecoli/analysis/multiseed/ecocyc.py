...
def _genes_transform(
    ...
    def callback(*outputs) -> pl.DataFrame:
        ...
        if observable_ids is not None and len(observable_ids):
            mrna_df: pl.LazyFrame = mrna_df.filter(pl.col("gene names").is_in(observable_ids))
