"""
MIGRATED: Interface for configuring and running **single-cell** E. coli simulations.
...
import ecoli.composites.environment.lattice  # Environment composer for spatial environment sim
from ecoli.library.logging_tools import write_json
from ecoli.shared.registry import ecoli_core
# from ecoli.processes import process_registry
from ecoli.migrated.cell_division import DivisionDetected
from ecoli.processes.registries import topology_registry
from ecoli.composites.ecoli_configs import CONFIG_DIR_PATH
from ecoli.library.schema import not_a_process
from ecoli.composites_v2.ecoli_master import Ecoli
...
class EcoliSim:
    ...
    def _retrieve_processes(
        ...
        :py:data:`~process_bigraph.ProcessTypes` (processes are
        registered in ``ecoli/migrated/__init__.py``).
        ...
        for process_name in list(processes.keys()) + list(add_processes):
            ...
            process_class = ecoli_core.process_registry.access(process_name)
            if not process_class:
                raise ValueError(
                    f"Unknown process with name {process_name}. "
                    "Did you call ecoli_core.process_registry.register() in "
                    "ecoli/migrated/__init__.py?"
                )
