...
# run_colony_ray workflow against it.
#
# This head node is the *driver*: small instance, runs
# runscripts/aws/ec2_cluster_ray_colony.py which uses
...
CONFIG_RELPATH="${CONFIG_RELPATH:-configs/colony_ray_aws.json}"
SESSION="${SESSION:-vecoli-v2-ray-colony}"
...
echo "=== bootstrap_head_ray_colony.sh start  $(date -u +%Y-%m-%dT%H:%M:%SZ)  session=$SESSION ==="

# All per-run knobs (n_init_sims, generations, max_duration) live in
# the config — see configs/comparison_10s_16g_v2_ray_aws.json.
# ec2_cluster_ray_colony.py walks the inherited chain and reads them from
# there, then forwards CONFIG_RELPATH to run_run_colony_ray.py
...
# Ray is intentionally NOT installed on the driver: ec2_cluster_ray_colony.py
...
# The driver is ec2_cluster_ray_colony.py — provisions cluster, runs
...
# ec2_cluster_ray_colony.py can pass it through as --experiment-id to
# run_run_colony_ray.py (vecoli_aws.sh assigns one per
