process runParca {
    ...
    script:
    """
    PYTHONUNBUFFERED=1 python ${params.projectRoot}/runscripts/parca.py --config "$config" -o "\$(pwd)"
    """
...
process analysisParca {
    ...
    script:
    """
    PYTHONUNBUFFERED=1 python ${params.projectRoot}/runscripts/analysis.py --config "$config" \
