...
_MAXED_OUT = (
    "GLP_EOBJ{}L: Dual simplex: The objective function being"
...
class NetworkFlowGLPK(NetworkFlowProblemBase):
    ...
    def _getVar(self, flow):
        if flow in self._flows:
            ...
            raise ValueError(
                'Equality constraints already built. Unable to add new flow: "{}".'.format(
                    flow
                )
            )
        else:
    ...
    def getFlowRates(self, flows):
        ...
        return np.array(
            [
                self._col_primals[self._flows[flow]] if flow in self._flows else None
                for flow in flows
            ]
        )
    ...
    def getSMatrix(self):
        if not self._eqConstBuilt:
            raise RuntimeError(
                "Equality constraints not yet built. Finish construction of the problem before accessing S matrix."
            )
    ...
    def getFlowNames(self):
        if not self._eqConstBuilt:
            raise RuntimeError(
                "Equality constraints not yet built. Finish construction of the problem before accessing flow names."
            )
    ...
    def getMaterialNames(self):
        if not self._eqConstBuilt:
            raise RuntimeError(
                "Equality constraints not yet built. Finish construction of the problem before accessing material names."
            )
    ...
    def buildEqConst(self):
        ...
        for materialIdx, (material, pairs) in enumerate(
            sorted(self._materialCoeffs.items())
        ):
    ...
    def _cache_glp_arrays(self):
        ...
        for material in self._materialCoeffs:
            ...
            self._flow_locations[material] = {
                idx: i + 1  # +1 for swiglpk indexing
