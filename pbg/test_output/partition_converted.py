...
class Requester(Step):
    ...
    def update_condition(self, timestep, states):
        ...
        if states["next_update_time"] <= states["global_time"]:
            if states["next_update_time"] < states["global_time"]:
                warnings.warn(
                    f"{self.name} updated at t={states['global_time']} instead of t={states['next_update_time']}. Decrease the timestep of the global_clock process for more accurate timekeeping."
                )
            return True
        return False
    ...
    def ports_schema(self):
        ...
        ports["request"] = {
            "bulk": {"_updater": "set", "_divider": "null", "_emit": False}
        }
        ports["process"] = {
            "_default": tuple(),
            "_updater": "set",
...
class Evolver(Step):
    ...
    def update_condition(self, timestep, states):
        ...
        if states["next_update_time"] <= states["global_time"]:
            if states["next_update_time"] < states["global_time"]:
                warnings.warn(
                    f"{self.name} updated at t={states['global_time']} instead of t={states['next_update_time']}. Decrease the timestep for the global clock process for more accurate timekeeping."
                )
            return True
        return False
...
class PartitionedProcess(BaseProcess):
